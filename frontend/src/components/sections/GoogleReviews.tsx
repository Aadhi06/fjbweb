...
function ReviewCard({ review, expanded, onToggle }: { review: GoogleReview; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-border hover:border-primary/30 hover:shadow-xl transition-all duration-200 flex flex-col h-full min-w-0 flex-shrink-0">
      <StarRating rating={review.rating} />
      <div className="mt-4 mb-4 flex-1">
        <p className={`text-sm text-black/70 leading-relaxed ${!expanded && needsTruncation ? "line-clamp-4" : ""}`}>
...
export function GoogleReviews() {
  ...
  useEffect(() => {
    if (isPaused || reviews.length <= 4) return;
  ...
  }, [isPaused, reviews.length, scrollTo]);
  ...
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading label="Customer Reviews" title="What Our Customers Say" description="Trusted by thousands across the UK with verified Google reviews." />
        ...
        >
          ...
          {reviews.length > 4 && (
            <>
              <button
                ...
                className="hidden lg:flex absolute -left-5 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white border border-border shadow-lg items-center justify-center text-black hover:bg-gray-50 disabled:opacity-30 disabled:cursor-default transition-all"
              ...
              <button
                ...
                className="hidden lg:flex absolute -right-5 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white border border-border shadow-lg items-center justify-center text-black hover:bg-gray-50 disabled:opacity-30 disabled:cursor-default transition-all"
          ...
          {reviews.length <= 4 ? (
            ...
            >
              {reviews.map((review, i) => (
                <div key={i} className="w-[280px] sm:w-[300px] flex-shrink-0 snap-start">
                  <ReviewCard
                    review={review}
                    expanded={expandedIdx === i}
                    onToggle={() => setExpandedIdx(expandedIdx === i ? null : i)}
